...
import { CourseExpandableList } from "../course-expenable-list/course-expandable-list";
...
export const UsersTable = () => {
  ...
  const sortTable = async (pagination, filters, sorter) => {
    ...
    const { data } = await postRequest(`/Users/GetSortedUsers`, { searchString: lastSearchData, order: order ? order : "", field, pageSize, current });
    setUsers(data.users);
  ...
  const onFinish = async (values) => {
    ...
    if (values.searchData) {
      ...
      const { status, data } = await postRequest(
        `/Users/GetSortedUsers`,{searchString: values.searchData, order: order ? order : "", field, pageSize, current:1 });  
    ...
    const clearValue = async () => {
      ...
      const { data } = await postRequest(
        `/Users/GetSortedUsers`,{searchString : '', order:'ascend', field:'id', pageSize, current:0 });
  ...
  useEffect(() => {
    const getUsers = async () => {
      const { status, data } = await postRequest(
        "/Users/GetAllUsersWithFullInfo",{current: pagination.current, pageSize: pagination.pageSize, total: pagination.total}
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "nickName",
      sorter: true,
      sortOrder: sorter.columnKey === "nickName" && sorter.order
    },
    {
      title: "Email",
      dataIndex: "email",
